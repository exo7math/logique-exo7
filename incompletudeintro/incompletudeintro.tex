...
\begin{itemize}
    \item Une théorie $\mathcal{T}$ est \defi{incohérente} s'il existe une formule fermée $\mathcal{Q}$ telle que :
\[
...
Elle est dite \defi{cohérente} sinon.
    ...
    \item Notez bien que la définition de \og{}incohérente\fg{} n'est pas la négation de \og{}incomplète\fg{} car la négation d'un \og{}et\fg{} est un \og{}ou\fg{}.

    \item Ainsi, une théorie incohérente prouve $\mathcal{T} \lturn (\mathcal{Q} \land \lnot\mathcal{Q})$ donc $\mathcal{T} \lturn \lantitauto$.
    ...
    \item Une théorie incohérente n'a donc pas d'intérêt puisqu'elle ne peut pas avoir de modèle. En effet, si elle possédait un modèle, et comme $\mathcal{T} \lturn \mathcal{Q}$ et $\mathcal{T} \lturn \lnot\mathcal{Q}$, par le théorème de validité, alors $\mathcal{Q}$ et $\lnot\mathcal{Q}$ devraient être toutes les deux vraies, ce qui fournit une contradiction.
...
L'\defi{hypothèse du continu}\index{hypothese du continu@hypothèse du continu} est un énoncé beaucoup plus concret qui est, lui aussi, un énoncé indécidable (ni $\mathcal{HC}$ ni $\lnot\mathcal{HC}$ ne sont démontrables dans $\mathcal{ZFC}$).
