...
\insertvideo{dKWabwM0YC4}{partie 3.1. Évaluation des formules}

\insertvideo{70ZUOIc-vyw}{partie 3.2. Un système complet de connecteurs}

\insertvideo{b0DcCQW3wQ4}{partie 3.3. Preuve du théorème}
...
Ainsi dans ce chapitre, les valeurs booléennes sont $\Bb = \{ \lzero, \lone \}$ avec $\lone$ pour \ltrue{} (Vrai) et $\lzero${} (Faux).
...
On appelle \defi{fonction abstraite}\index{fonction!abstraite} une fonction :
\[ 
f : \{\lzero,\lone\}^n \to \{\lzero,\lone\}
\]
Soit $x = (\epsilon_1,\ldots,\epsilon_n) \in \{\lzero,\lone\}^n$. C'est un vecteur dont toutes les coordonnées sont $\lzero$ ou $\lone$. De plus $f(x)$ vaut $\lzero$ ou $\lone$.
...
L'\defi{évaluation}\index{evaluation@évaluation} de $\mathcal{P}$ en $x = (\epsilon_1,\ldots,\epsilon_n) \in \{\lzero,\lone\}^n$ consiste 
...
\begin{exemple}
	...
	Par exemple en $x=(\lzero,\lone,\lzero)$ on attribue $(\lfalse,\ltrue,\lfalse)$ aux variables $(P_1,P_2,P_3)$ et alors la formule $\mathcal{P}$ s'évalue à $(\lfalse \lor \ltrue) \land (\lnot \lfalse \lor \lfalse)$ et vaut donc $\ltrue$. Ainsi $f_{\mathcal{P}}(\lzero,\lone,\lzero) = \lone$.
...
Dans l'exemple précédent on avait une formule $\mathcal{P}$  dont la fonction associée $f_{\mathcal{P}}$ était la même qu'une fonction abstraite définie auparavant.
...
Si la réponse était négative, cela voudrait dire que nos connecteurs ne sont pas suffisants pour modéliser toutes les fonctions et qu'il faudrait inventer de nouveaux connecteurs pour réaliser certaines fonctions.
...
$\lone\downarrow\lone=\lzero$. 
Mais en fait $P \downarrow Q$ se réalise par la formule $\lnot(P \lor Q)$. Ainsi on n'a pas vraiment besoin d'utiliser un symbole spécial pour cette opération.
...
\begin{itemize}
	...
	\index{ou exclusif@\tconnector{xor} ou exclusif}
...
On verra la preuve dans la prochaine section. Cela signifie que, quelle que soit $n \ge 1$ et quel que soit $f : \{\lzero,\lone\}^n \to \{\lzero,\lone\}$, alors il existe une formule $\mathcal{P}$ ne faisant intervenir que les connecteurs $\lnot$ et $\land$, telle que la fonction $f_{\mathcal{P}}$ issue de $\mathcal{P}$ soit exactement cette fonction $f$.
...
Une autre façon de comprendre ce théorème est de dire que pour n'importe quelle formule (avec n'importe quels connecteurs), on peut trouver une formule logiquement équivalente qui ne contient que les connecteurs $\lnot$ et $\land$. On rappelle que deux formules $\mathcal{P}$ et $\mathcal{Q}$ sont logiquement équivalentes (noté $\mathcal{P} \equiv \mathcal{Q}$) si elles ont la même table de vérité.

%--------------------------------------------------------------------
\textbf{Question pas si bête !}
Si on n'a besoin que de $\{\lnot, \land\}$ pour réaliser toutes nos fonctions, alors pourquoi ajoute-t-on les autres connecteurs $\lor$, $\limplies$, $\lequiv$ ?
...
\begin{enumerate}
	...
	Par exemple $P \land \lnot Q$, $\lnot Q \land P \land \lnot Q$ et $(\lnot \lnot P) \land (\lnot Q)$ réalisent la même fonction.
	C'est encore moins unique si on autorise tous les connecteurs $\{\lnot, \land, \lor, \limplies, \lequiv\}$, par exemple $P \limplies Q$, $\lnot P \lor Q$ et $\lnot (P \land \lnot Q)$ sont des formules qui réalisent la même fonction.
	...
	\]
	Problème : il y a un $\lor$ (\tor) dans la formule, alors qu'on voudrait utiliser seulement $\lnot$ et $\land$, on verra la solution dans le paragraphe suivant.
\end{enumerate}
...
\]
Ainsi, si on a une formule quelconque, le théorème nous permet d'obtenir une formule logiquement équivalente uniquement avec $\lnot$ et $\land$. On peut alors remplacer chaque $\land$ via l'équivalence logique ci-dessus afin d'obtenir une formule ne contenant que des $\lnot$ et des $\limplies$.
...
Puisque l'on a vu que $\lor$ est engendré par $\{ \lnot, \land \}$ via $P \lor Q \equiv \lnot(\lnot P \land \lnot Q)$, on aura bien prouvé que $\mathcal{C}_0 = \{\lnot, \land\}$ est un système complet de connecteurs.
...
\begin{itemize}
	...
	$\mathcal{P} = P_1 \land \lnot P_1$ (toujours faux) et on a bien $f_{\mathcal{P}} = f$.
	
	\item Sinon, on note $\{a_1,\ldots,a_k\}$ les éléments de  $\{\lzero,\lone\}^n$
	tels que $f(x) = \lone$. Alors $f = f_{a_1,\ldots,a_k}$, d'après l'étape 2.
\end{itemize}
...
\begin{itemize}
	...
	\]
	s'appelle la \defi{forme normale disjonctive}.\index{forme normale!disjonctive}
	
	Cette forme normale est intéressante car cette décomposition est unique à l'ordre près (on a bien sûr $P \lor Q  \equiv Q \lor P$ et $P\land Q \equiv Q \land P$).
	
	\item Une variante est la \defi{forme normale conjonctive}\index{forme normale!conjonctive} : 
	\[
	\mathcal{Q} : \qquad \bigwedge_{j=1}^k \ \bigvee_{i=1}^n \delta_{i,j} P_i
	\]	
	où $\delta_{i,j} \in \{\lzero,\lone\}$.
	Son écriture est également unique (à l'ordre près). 
