...
\begin{itemize}
    ...
    \item Une théorie $\mathcal{T}$ est \defi{noncohérente} s'il existe une formule $\mathcal{Q}$ telle que :
\[
\mathcal{T} \lturn \mathcal{Q} \qquad \text{ et } \qquad \mathcal{T} \lturn \lnot \mathcal{Q}.
\]
Elle est dite \defi{cohérente} sinon.\index{theorie@théorie!coherente@cohérente}

    \item De façon équivalente, $\mathcal{T}$ est non cohérente si $\mathcal{T} \lturn \bot$ et alors $\mathcal{T}$ prouve n'importe quelle formule (et sa négation), par la règle d'explosion.
Une théorie non cohérente n'a donc aucun intérêt.

    \item Si $\mathcal{T}$ admet un modèle (c'est-à-dire $\ldoubleturn_\mathcal{S} \mathcal{T}$ pour une certaine structure $\mathcal{S}$), alors $\mathcal{T}$ est cohérente via le théorème de validité.
\end{itemize}
...
\[
\mathcal{T} \not\lturn \mathcal{Q} \qquad \text{ et } \qquad \mathcal{T} \not\lturn \lnot \mathcal{Q}.
\]
...
\[
\mathcal{T} \lturn \mathcal{Q} \qquad \text{ ou } \qquad \mathcal{T} \lturn \lnot \mathcal{Q}.
\]
...
\begin{definition}
    ...
    Une théorie $\mathcal{T}$ est \defi{$\omega$-non cohérente} s'il existe une formule $\mathcal{Q}(x)$ (ayant $x$ pour seule variable libre) telle que :
    \[
...
\begin{itemize}
    ...
    \item $\mathcal{T}$ non cohérente implique $\mathcal{T}$ $\omega$-non cohérente.
\end{itemize}

Justification : si $\mathcal{T}$ est non cohérente, alors $\mathcal{T}$ prouve n'importe quoi, donc prouve les formules de la définition de la $\omega$-non cohérence. L'autre sens s'obtient par contraposition.
...
\subsection{La formule de Gödel}

Rappelons que la formule $\mathcal{E}\text{st-preuve}(y,x)$
représente dans $\mathcal{PA}$ le prédicat \og{}$y$ est le code d'une preuve de la formule de code $x$\fg{}. On définit alors :
\[
\mathcal{E}\text{st-démontrable}(x)
\ : \quad
\exists y \ \ \mathcal{E}\text{st-preuve}(y,x)
\]

Considérons la formule
\[
\mathcal{P}(x) \quad : \quad
\lnot\mathcal{E}\text{st-démontrable}(x)
\]
...
Ainsi, dans $\mathcal{PA}$, la formule $\mathcal{G}$ affirme qu'elle n'est pas démontrable. Autrement dit, elle exprime :
\og{}Je ne suis pas démontrable.\fg{}
...
Conclusion : si $\mathcal{PA} \lturn \mathcal{G}$ alors $\mathcal{PA} \lturn \lnot \mathcal{G}$, ce qui implique que $\mathcal{PA}$ est non cohérente. Or on a supposé $\mathcal{PA}$ $\omega$-cohérente, donc cohérente (cette hypothèse simple suffit pour cette première étape). On obtient une contradiction.
...
\]
Cela implique que $\mathcal{PA}$ est $\omega$-non cohérente, en contradiction avec notre hypothèse.
...
\begin{itemize}
    ...
    \item Une théorie $\mathcal{T}$ a des \defi{axiomes récursifs}\index{axiomes!recursifs@récursifs} (resp. récursifs primitifs) si $A = \{ \ulcorner \mathcal{P} \urcorner \mid \mathcal{P} \text{ est un axiome de } \mathcal{T} \}$, l'ensemble des nombres de Gödel des axiomes de $\mathcal{T}$, est un ensemble récursif (resp. récursif primitif).
\end{itemize}
...
Cela permet donc de construire le prédicat $\text{est\_preuve}(N, n)$ : $\Nn^2 \to \{\ltrue,\lfalse\}$ récursif primitif, puis de le représenter par une formule de $\mathcal{L}_0$, $\mathcal{E}\text{st-preuve}(y, x)$.
