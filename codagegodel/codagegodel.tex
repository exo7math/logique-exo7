...
$x_1 / x$ & $x_2 / y$ & $x_3 / z$ & $x_4$ & $\cdots$ & $x_i$ & $\cdots$\\ 
$2$ & $4$ & $6$ & $8$ & $\cdots$ & $2i$ & $\cdots$
\end{tabular}
\end{center} 
...
Si l'on veut vraiment un algorithme dans le langage $\mathcal{L}_0$, $n$ doit être écrit sous la forme du numéral $\overline{n} = SS \dots S 0$. 
...
Il faut maintenant écrire cela en termes de nombres de Gödel, c'est-à-dire qu'il nous faut une fonction \ci{substitution}($\ulcorner \mathcal{P} \urcorner$, $\ulcorner x \urcorner$, $n$) $: \Nn^3 \to \Nn$, qui renvoie $\ulcorner \mathcal{P}(\overline{n}) \urcorner$, le nombre de Gödel de la substitution.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Preuve informelle du théorème d'incomplétude}
...
\begin{itemize}
    ...
    \item Enfin, pour un $n$ donné on peut discuter si la formule fermée $\mathcal{Q}(n)$ est démontrable ou pas. C'est exactement ce que fait la formule $\mathcal{P}(n)$ : \og{}La formule $\mathcal{Q}(n)$ n'est pas démontrable.\fg{}
\end{itemize}
