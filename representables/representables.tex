...
\begin{itemize}
    ...
    \item Il faut comprendre la formule $\mathcal{P}$ comme le graphe de $f$.
    Pour un graphe d'une fonction de $\Rr \to \Rr$ on a $y=f(x)$ ssi $(x,y)$ appartient au graphe de $f$.
    Ici pour une fonction de $\Nn^p \to \Nn$, on a $y=f(\mathbf{x})$ ssi $\mathcal{P}(\mathbf{x},y)$.
    C'est une façon équivalente de définir la fonction.
\end{itemize}
...
\[
\mathcal{P}_h(\mathbf{x},y) \ : \ \exists w_1 \dots \exists w_q \quad \mathcal{P}_0(w_1, \dots, w_q, y) \land \mathcal{P}_1(\mathbf{x}, w_1) \land \cdots \land \mathcal{P}_q(\mathbf{x}, w_q)
\]
...
\begin{proof}
    ...
    \[ \mathcal{P}_h(x,y) \ : \quad \exists w \ \ \mathcal{P}_0(w, y) \land \mathcal{P}_1(x, w)  \]
    ...
    \begin{itemize}
        ...
        Donc $\mathcal{PA} \lturn \mathcal{P}_h(n, m)$.
        ...
        Ainsi, supposons qu'on ait $\mathcal{P}_h(n, y)$. Donc il existe $w$ tel que $\mathcal{P}_0(w, y)$ et $\mathcal{P}_1(n, w)$.
