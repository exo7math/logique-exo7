...
\objectifs{La logique \og{}Vrai/Faux\fg{}, aussi appelée logique propositionnelle, s'attache à étudier quelles valeurs, parmi \og{}Vrai\fg{} ou \og{}Faux\fg{}, peut prendre une formule définie à partir des connecteurs logiques \tand, \tor, \tnot\ldots}
...
Nous définissons deux valeurs \ltrue{} (Vrai) et \lfalse{} (Faux), appelées \defi{valeurs booléennes} ou \defi{valeurs de vérité}.
On utilisera aussi la notation \lzero{} (Faux) et \lone{} (Vrai).
La notation anglo-saxonne est \ltruetrue{} (\emph{True}) et \lfalsefalse{} (\emph{False}).


Une \defi{variable propositionnelle} est une variable qui peut prendre soit la valeur \ltrue{} (Vrai), soit la valeur \lfalse{} (Faux).
...
\begin{itemize}
	\item \defi{Négation} $\lnot P$. \quad $\lnot P$ est vraie si et seulement si $P$ est fausse.
    
	\item \defi{Conjonction} $P \land Q$. \quad  $P \land Q$ est vraie si et seulement si $P$ et $Q$ sont vraies.
    
	\item \defi{Disjonction} $P \lor Q$. \quad $P \lor Q$ est vraie si et seulement si $P$ est vraie ou $Q$ est vraie.
\end{itemize}
...
Évidemment, il y a un problème de logique dans nos définitions, car par exemple nous avons défini le connecteur \og{}$\land$ \fg{} (\tand) à l'aide du mot français \og{}et\fg{}. Pour remédier à ce problème, on définit l'action de chacun de ces connecteurs par sa \defi{table de vérité} :
...
L'\defi{implication} $P \limplies Q$ est définie par :
\mybox{
...
\begin{itemize}	
	...
	et \og{}$3 \times 3 = 7$\fg{} $\limplies$ \og{}$2+2=4$\fg{}
...
L'\defi{équivalence} $P \lequiv Q$ est définie par la double implication $P \limplies Q$ et $Q \limplies P$ :
\mybox{
...
Le connecteur $\lnot$ est un connecteur \defi{unaire}, car il s'applique à un seul paramètre (comme une fonction d'une variable) alors que les autres sont des connecteurs \defi{binaires} (par exemple on pourrait écrire $P \land Q$ sous la forme $\land(P,Q)$ comme une fonction de deux variables).
...
Nous disposons de \defi{variables propositionnelles} $P$, $Q$, $R$\ldots{} chacune pouvant prendre la valeur Vrai ou Faux. À l'aide des connecteurs, on compose une \emph{formule} $\mathcal{P}$, par exemple :
\[
...
On considère un \defi{alphabet} formé par :
\begin{itemize}
...
Une \defi{formule} est un mot fini, formé à partir des caractères de cet alphabet ; ce mot doit être correctement parenthésé et cohérent avec l'usage des connecteurs.
...
Pour une formule $\mathcal{P}$ qui dépend de deux variables propositionnelles $P$ et $Q$, on peut déterminer quand la formule prend la valeur vraie ou fausse selon les valeurs attribuées à $P$ et $Q$. Cela se résume sous la forme d'une \defi{table de vérité} de la formule $\mathcal{P}$ :
...
une \defi{distribution de valeurs} est un choix \ltrue/\lfalse{} pour la variable $P_1$, un choix \ltrue/\lfalse{} pour la variable $P_2$,\ldots{}
On obtient alors une \defi{évaluation} \ltrue/\lfalse{} pour la formule $\mathcal{P}$.
...
\begin{definition}
	Deux formules $\mathcal{P}$ et $\mathcal{Q}$ sont \defi{logiquement équivalentes} si elles ont la même table de vérité. On note alors :
...
Il y a exactement deux valeurs booléennes $\{\ltrue, \lfalse\}$ (ou $\{\lzero, \lone\}$), ni plus, ni moins ! Ce qui fait que si une variable $P$ ou une formule $\mathcal{P}$ n'est pas évaluée à Faux, c'est qu'elle est vraie. Et réciproquement. Il n'y a donc pas de réponse \og{}peut-être\fg{}.
Cela a les conséquences suivantes :
\begin{itemize}
	\item \defi{Principe du tiers exclu} :
...
On verra un peu plus tard dans la Section \ref{sec:formulesbis} une nouvelle écriture de ce qui est vrai/faux :
...
\begin{exemple}
 Soit $n$ un entier :
\[
n \text { impair} \limplies n \text{ premier}
\]
...
\begin{itemize}
	\item si $n=3$, alors \og{}$n \text { impair} \limplies n \text{ premier}$\fg{} est vraie, car $P$ : \og{}$3$ impair\fg{} et $Q$ : \og{}$3$ premier\fg{} sont vraies.
	
	\item si $n=15$, alors \og{}$n \text { impair} \limplies n \text{ premier}$\fg{} est fausse, car $P$ : \og{}$15$ impair\fg{} est vraie alors que $Q$ : \og{}$15$ premier\fg{} est fausse.
	
	\item si $n=4$, alors \og{}$n \text { impair} \limplies n \text{ premier}$\fg{} est vraie ! En effet $P$ : \og{}$4$ impair\fg{} est fausse et donc $P \limplies Q$ est vraie. 
\end{itemize}
...
\begin{itemize}
	...
	\item $n=2$. Le \defi{ou exclusif} $\oplus$/\tconnector{xor} est un autre connecteur binaire :
	\mycenterline{$P \oplus Q$ est vrai si exactement l'une des variables est vraie.}
	
	\item $n=2$. Le connecteur $\uparrow$/\tconnector{nand}, appelé aussi symbole de Sheffer est défini comme la négation du \tand :
...
\begin{definition}
	\sauteligne
	\begin{itemize}
		\item Une \defi{tautologie} est une formule $\mathcal{P}$ qui s'évalue toujours à Vrai, quelle que soit la distribution des valeurs de vérité.
...
\begin{exemple}
	...
	\begin{enumerate}
		...
		\item $(P \lor Q \lor R) \lor (\lnot P \lor \lnot Q \lor \lnot R)$ est une tautologie.
...
Pour se rapprocher de ce qui se passe au cœur d'un ordinateur on peut considérer les connecteurs comme des portes logiques. Ici les valeurs de vérité sont $\{\lzero, \lone\}$.
...
On peut aussi adopter la \defi{notation polonaise}, c'est-à-dire écrire $\land P Q$ (comme une fonction de deux variables $\land(P,Q)$) au lieu de $P \land Q$.
...
\begin{exercicecours}
	...
	\]
	Traduiser ces équivalences par une phrase en français.
\end{exercicecours}
